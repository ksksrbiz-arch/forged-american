...
export function VariantSelector({ product, variants }: { product: Product; variants: Variant[] }) {
  ...
  // Distinct option lists, preserving DB order
  const sizes = useMemo(
  ...
  // Per-size availability for current color
  ...
  // Per-color availability for current size
  ...
  return (
    <div className="flex flex-col gap-5">
            ...
            {colors.map((c) => {
              ...
              return (
                <button
                  key={c}
                  onClick={() => setColor(c)}
                  className={`px-4 py-2 border tracking-wider-3 uppercase text-xs font-display transition-colors ${
                    selected
      ...
      {sizes.length > 1 && (
        <div>
          <div className="flex items-baseline justify-between mb-2">
            ...
            <a href="#size-guide" className="text-xs text-muted hover:text-accent underline">Size guide</a>
          </div>
            ...
            {sizes.map((s) => {
              ...
              return (
                <button
                  key={s}
                  onClick={() => setSize(s)}
                  className={`py-3 border tracking-wider-3 uppercase text-sm font-display transition-colors ${
                    selected
                      ? 'border-accent bg-accent text-white'
                      : avail
                      ? 'border-border hover:border-accent text-ink'
      ...
      {lowStock && (
        <p className="text-accent-2 text-xs font-display tracking-wider-3 uppercase">
          ★ Only {matched?.inventory} left
        </p>
      )}
